...
const SetlistManager = ({ songs = [], setlists = [], onCreateSetlist, onUpdateSetlist, onDeleteSetlist, onViewSetlist, onAddSongToSetlist, onRemoveSongFromSetlist }) => {
  ...
  const [localSetlistSongs, setLocalSetlistSongs] = useState([]) // Local state for editing
  const [searchTerm, setSearchTerm] = useState('')
  
  // Guard against undefined or non-array props
  const safeSongs = Array.isArray(songs) ? songs : [];
  const safeSetlists = Array.isArray(setlists) ? setlists : [];
  ...
  // Initialize local songs when editing a setlist
  useEffect(() => {
    if (editingSetlist?.songs) {
      setLocalSetlistSongs([...editingSetlist.songs])
    } else {
      setLocalSetlistSongs([])
    }
  }, [editingSetlist?.id]) // Only when we start editing a different setlist
  ...
  const handleCreateSetlist = async () => {
    ...
    try {
      const createdSetlist = await onCreateSetlist(newSetlistData)
      setNewSetlistName('')
      setIsCreating(false)
      
      // Use the real setlist returned from the backend
      if (createdSetlist) {
        setEditingSetlist(createdSetlist)
      }
    } catch (error) {
  ...
  const handleAddSongToSetlist = async (song) => {
    ...
    // Check if song is already in local songs
    if (localSetlistSongs.find(s => s.id === song.id)) {
      alert('This song is already in the setlist')
      return
    }

    try {
      await onAddSongToSetlist(editingSetlist.id, song)
      // Immediately update local state
      setLocalSetlistSongs(prev => [...prev, song])
    } catch (error) {
      console.error('Failed to add song to setlist:', error)
      if (error.message && error.message.includes('already in this setlist')) {
        alert('This song is already in the setlist')
      } else {
  ...
  const handleRemoveSongFromSetlist = async (songId) => {
    if (!editingSetlist) return

    try {
      await onRemoveSongFromSetlist(editingSetlist.id, songId)
      // Immediately update local state
      setLocalSetlistSongs(prev => prev.filter(song => song.id !== songId))
    } catch (error) {
      console.error('Failed to remove song from setlist:', error)
      alert('Failed to remove song from setlist. Please try again.')
    }
  }
  ...
  const handleDragStart = (e, index) => {
    ...
    setReorderedSongs([...localSetlistSongs]) // Start with current local order
  ...
  const handleDragOver = (e, index) => {
    ...
    const newSongs = [...localSetlistSongs]
  ...
  return (
    <div className="setlist-manager">
      <div className="setlist-header">
        <h2>🎵 Setlists ({safeSetlists.length})</h2>
        {!isCreating && (
          <button
          ...
          <div className="setlist-editor-modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="setlist-editor-modal-header">
              ...
              <div className="editor-actions" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <span className="song-count">{localSetlistSongs.length} songs</span>
                <button
              ...
              <div className="setlist-songs">
                ...
                {localSetlistSongs.length === 0 ? (
                  ...
                  <div className="setlist-songs-list">
                    {(reorderedSongs.length > 0 ? reorderedSongs : localSetlistSongs).map((song, index) => {
                      // Find the original index of this song to determine if it's being dragged
                      const originalIndex = localSetlistSongs.findIndex(s => s.id === song.id)
                  ...
                  {filteredSongs.length === 0 ? (
                    ...
                    <div className="songs-grid">
                      {filteredSongs.map(song => {
                        const isInSetlist = localSetlistSongs.find(s => s.id === song.id)
                        return (
                          <div
                            key={song.id}
